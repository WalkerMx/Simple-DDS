...
Public Class Form1
    ...
    Private Sub LoadStandardImage(Path As String)
        Dim DetectedFaces As String() = DetectCubeMap(Path)
        If DetectedFaces IsNot Nothing Then
            CubeMode = True
            FilePaths = DetectedFaces
            Dim TempCubeMaps(5) As Bitmap
            For i As Integer = 0 To 5
                TempCubeMaps(i) = CType(Image.FromFile(DetectedFaces(i)), Bitmap)
            Next
            LoadCubeMaps(CubeFaces, TempCubeMaps)
    ...
    Public Function DetectCubeMap(SourceFilePath As String) As String()
